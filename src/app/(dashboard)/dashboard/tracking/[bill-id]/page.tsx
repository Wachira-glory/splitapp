  ...
  try {
    ...
    if (authError || !user) {
      setError("Please log in to view this bill.");
      router.push('/login');
      return;
    }

    // 2. Fetch the bill ensuring it belongs to THIS user
    // The .eq("user_id", user.id) is our security layer
    ...
    if (billError || !billData) {
      console.error("Fetch bill error:", billError);
      setError("Bill not found or you do not have permission to view it.");
      return;
    }

    // 3. Fetch participants for this specific bill
    // Since we already verified the bill belongs to the user, 
    // we can safely pull the participants linked to it.
    ...
    console.error("Unexpected error in fetchBill:", err);
    setError("An unexpected error occurred while loading the tracker.");
  } finally {
    setLoading(false);
  }
};
